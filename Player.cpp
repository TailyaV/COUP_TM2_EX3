...
void Player::gather(){
    ...
    if((game->getTotalTurn() - st) >= game->getCurrNumOfPlayers() || game->getTotalTurn() <= game->getCurrNumOfPlayers()){
        this->coins = coins + 1;
        game->nextTurn();
    }
    else{
    ...
    if(this->getCoins() >= 4){
        ...
        for(int i = 0; i < game->getnumOfPlayers(); ++i){
            if(game->getPlayersList()[i]->getRole() == Role::Judge){
                Judge* j = static_cast<Judge*>(game->getPlayersList()[i]);
...
void Player::arrest(int arestedPlayer){
    ...
    if(game->getTotalTurn() -  spyPreventArrest > game->getCurrNumOfPlayers() || spyPreventArrest == -1){
        if(arrestTurn == -1){
        ...
        if(!(lastArrested == arestedPlayer && game->getTotalTurn() -  arrestTurn <= game->getCurrNumOfPlayers())){
...
void Player::sanction(int sanctionedPlayer){
    ...
    if(this->coins >= 3){
        ...
        if(game->getPlayer(sanctionedPlayer)->getRole() == Role::Baron){
            Baron* b = static_cast<Baron*>(game->getPlayer(sanctionedPlayer));
            b->compensation();
        }
...
void Player::cuop(int coupedPlayer){
    ...
    for(int i = 0; i < game->getnumOfPlayers(); ++i){
        if(game->getPlayersList()[i]->getRole() == Role::General){
            General* g = static_cast<General*>(game->getPlayersList()[i]);
